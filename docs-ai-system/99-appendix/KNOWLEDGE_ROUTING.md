...
## AI-DRIVEN CRM DEMO / PHASE 3

Load:
- ai-system/MASTER_PLAN.md (Phase 3 section)
- ai-system/ACTIVE_TASK.md
- README.md (platform philosophy — efficiency, observability)
...
- project_devops/platform/composition/docker-compose.apps.yml
- project_devops/apps/ai-crm-pipeline/ARCHITECTURE.md (when exists)
- project_devops/apps/ai-crm-pipeline/docs/* (when exists)

Task-type mapping:
- **3.1 Design** → architecture docs only; no runtime changes
- **3.2–3.3 Code** → SERVICE CREATION rules + Phase 3 docs
- **3.6 Infra** → ENVIRONMENT / INFRASTRUCTURE WORK
- **3.7 CI** → CI/CD OR PIPELINE WORK
- **3.8 Obs** → OBSERVABILITY / DEBUGGING
- **3.9–3.10 Validation** → CONTRIBUTION + runbooks

Goal:
Integrate CRM demo as first-class platform services without breaking 6GB constraint.

---

## SHOPEE TOP-10 SEARCH / PHASE 5

Load:
- ai-system/AI_BOOT.md (Vagrant full-stack section)
- ai-system/ACTIVE_TASK.md
- project_devops/apps/shope-search/README.md
- project_devops/apps/shope-search/architecture.md
- project_devops/apps/shope-search/API_CONTRACT.md
- project_devops/apps/shope-search/shopee-search-api/
- project_devops/apps/shopee-search-ui/
- project_devops/platform/composition/docker-compose.apps.yml (shopee-search-*)
- project_devops/platform/ops/migrations/apply-shopee-db.sh
- Vagrantfile, .env.example (repo root)

Task-type mapping:
- **5.d*** code → shope-search API/UI; contract first
- **5.d5 / platform** → compose, Vagrant hosts, migrations, smoke
- **5.lab** → DEMO_RUNBOOK.md browser-only

Goal:
`vagrant up` brings up Shopee with CRM + platform; keyword → Top 10 on https://shopee-search.nano.platform
