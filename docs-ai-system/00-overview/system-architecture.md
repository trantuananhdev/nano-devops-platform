...
|-----------|---------|---------|
| **LLM Router** | Điều phối các agent role đến backend phù hợp | 🟢 **Ubuntu llama-server (Gemma 4)** (local): <br/>  - `planner` (lập kế hoạch), `executor` (điều phối tool), `reflector` (phản tư), `summary` (tổng hợp báo cáo)<br/><br/> 🔵 **Gemini Flash API** (remote/mock):<br/>  - `legal` (chuyên gia pháp lý), `financial` (chuyên gia tài chính), `ocr`, `critic` (kiểm duyệt), `tool_mock` (giả lập tool ERP/DOffice)<br/><br/>  - Circuit Breaker cho cả 2 backend<br/>  - Gemini API Key Rotation + Cooldown (tối ưu chi phí) |
