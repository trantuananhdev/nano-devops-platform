...
The platform uses a unified `cli.sh` tool to manage services across multiple Docker Compose modules.

### Start Services

**Start All Services**:
```bash
# Start all modules (Core, Observability, Apps)
./cli.sh up
```

**Start Specific Service**:
```bash
# Start a specific service using the CLI (wraps docker compose)
./cli.sh up <service-name>
```
...
```bash
./cli.sh logs <service-name>
```

### Manual Docker Compose Operations

If you need granular control over specific modules, use the corresponding compose file:

- **Core**: `project_devops/platform/composition/docker-compose.yml`
- **Observability**: `project_devops/platform/composition/docker-compose.observability.yml`
- **Apps**: `project_devops/platform/composition/docker-compose.apps.yml`

Example:
```bash
docker compose -f project_devops/platform/composition/docker-compose.observability.yml restart platform-prometheus
```
