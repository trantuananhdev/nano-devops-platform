#!/usr/bin/env bash
# One-off CRM leads schema fix (fixes GET /api/v1/leads 500 on older DBs).
set -euo pipefail

PG_CONTAINER="${PG_CONTAINER:-platform-postgres}"

docker exec "$PG_CONTAINER" psql -U postgres -d crm_db -v ON_ERROR_STOP=0 <<'EOSQL'
...
ALTER TABLE leads ADD COLUMN IF NOT EXISTS property_type VARCHAR(64);
ALTER TABLE leads ADD COLUMN IF NOT EXISTS location VARCHAR(255);
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO crm_user;
EOSQL

echo "CRM leads migration done."
