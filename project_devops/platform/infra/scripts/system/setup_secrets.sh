#!/bin/sh
# Secrets management script for the Nano DevOps Platform (Dev only)
...
log_info "=========================================="
log_info "Nano DevOps Platform - Secrets Management"
log_info "=========================================="

# Check if running as root (optional, for chown to deploy user if available)
if [ "$(id -u)" -ne 0 ]; then
    echo "Warning: Not running as root. Will skip chown to deploy user."
fi
...
generate_secret() {
    ...
    if [ ! -f "$file" ]; then
        ...
        chmod 600 "$file"
        # Ensure correct ownership if we can determine the user (e.g., deploy) and are root
...
# List of required secrets for core platform
...
# Secrets for HDTV AI Platform
generate_secret "hdtv_postgres_password"
generate_secret "minio_root_password"
generate_secret "meili_master_key"

log_info "=========================================="
log_info "Secrets management completed successfully!"
log_info "=========================================="
